...
class ConversationManager:
    ...
    def get_previous_conversations(self):
        ...
        try:
            ...
            conversations = [{"message": row[0]} for row in rows]
            conn.close()
    ...
    def save_conversation(self, user_message, ai_response, summary=None, embedding=None, agentic_role=None):
        """
        Save the conversation to the database with all fields in a single row.
        Also, retrain the Word2Vec model on the new message.
        """
        conn = sqlite3.connect(self.db_path)
        cursor = conn.cursor()
        ...
        # Generate an agentic role (AI job description) based on the summary and context
        if agentic_role is None:
            agentic_role = self.generate_agentic_role(summary, user_message, ai_response)
        
        # Insert the conversation into the database
        cursor.execute('''INSERT INTO conversations (timestamp, message, message_summary, embedding, agentic_role)
                          VALUES (?, ?, ?, ?, ?)''',
                       (timestamp, combined_message, summary, embedding, agentic_role))
        conn.commit()
        conn.close()
        logging.info("Conversation saved to the database.")

        # Retrain Word2Vec on the new message
        self.memory_handler.train_word2vec([combined_message])
    ...
    def generate_agentic_role(self, summary, user_message, ai_response):
        """
        Generate an agentic role (AI job description) based on the provided summary and context.
        The format follows the style: "Job Role - Job Description."
        """
        # Define the job role and description based on the context
        if "python" in user_message.lower() or "code" in user_message.lower():
            job_role = "Expert Python Coder"
            job_description = "Teach basic code and provide examples"
        elif "hello" in user_message.lower() or "hi" in user_message.lower():
            job_role = "Customer Service Rep"
            job_description = "Speak friendly and naturally"
        elif "data" in user_message.lower() or "analyze" in user_message.lower():
            job_role = "Data Analyst"
            job_description = "Analyze data and generate insights"
        elif "script" in user_message.lower() or "automate" in user_message.lower():
            job_role = "Scripting Specialist"
            job_description = "Automate tasks with scripts"
        else:
            job_role = "AI Assistant"
            job_description = "Provide helpful and accurate responses"

        # Format the role as "Job Role - Job Description"
        agentic_role = f"{job_role} - {job_description}"
